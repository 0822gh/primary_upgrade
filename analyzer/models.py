...
class APKAnalysis(models.Model):
    ...
    # 1차 업그레이드: 라벨 기반 검토 문장(요약)
    # List[Dict] 형태로 저장 (예: [{"label":..., "tag":..., "sentence":..., "evidence":[...]}])
    policy_sentence_labels = models.JSONField(default=list, blank=True, verbose_name="검토 문장(라벨 기반)")
